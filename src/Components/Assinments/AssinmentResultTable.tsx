...
export default function AssinmentResultTable({AssinmentResult,Assinment}:{AssinmentResult:AssinmentResultInterface,Assinment:AllAssinmentInterface}) {
  const { DetailsToggle, setDetailsToggle } = AssinmentHook({ Assinment });
  return (
    <>
      ...
      {DetailsToggle && <div className="w-full text-gray-700 border-black flex justify-center items-center">
        <div className="table w-full bg-green-200 border-1 border-blue-400">
          ...
          {AssinmentResult?.Assinments?.assinmentbody?.questions?.map(
            (e, i) => (
              <>
                <div
                  key={i}
                ...
                >
                  ...
                  <div className=" border-1 border-blue-400  table-cell text-center items-center ">
                    {AssinmentResult?.answersbody &&
                      JSON.parse(AssinmentResult?.answersbody).filter(
                      ...
                      <div className="bg-transparent border-none">
                        {JSON.parse(AssinmentResult?.answersbody).filter(
                          ...
                          <div className="flex flex-col bg-transparent border-none  items-center">
                            {e?.answer?.map((ans, i) => {
                              return (
                                ...
                                >
                                  {JSON.parse(
                                    AssinmentResult?.answersbody as unknown as string
                                  )
                                    ?.filter(
                                      (ele: { id: string }) => ele.id === e.id
                                    )[0]
                          ...
                          <div className="flex flex-col bg-transparent border-none justify-center items-center">
                            {JSON.parse(AssinmentResult?.answersbody)
                              .filter(
                                (ele: { id: string }) => ele.id === e.id
                              )[0]
                              ?.answer.map(

    ...
    <div className="w-[45%] flex gap-3 justify-start items-center p-1 text-center rounded">
                    ...
                    <p className="flex">
                    {
                        ...
                        <FaHeart title='أحببته' onClick={()=>DeleteLikeCommentHandeller(e.likes[e.likes?.findIndex((el)=>
                          ...
                          // (el.mangerId !== null )    &&   (el.mangerId == UserLogedData.id) ||
                          // (el.ownerId  !== null )    &&   (el.ownerId  == UserLogedData.id) 
                        )]?.id as unknown as string)}  className=" cursor-pointer text-red-500"/>
                  :
                  ...
                  :
                  <FaRegAngry title='أغضبني' onClick={()=>CreateLikeCommentHandeller(e.likes,'hate',Lesson?.Subjects?.id,comId as unknown as number,Lesson?.id,reId)} className="text-amber-800 hover:text-red-500 hover:animate-bounce cursor-pointer "/>
                    }

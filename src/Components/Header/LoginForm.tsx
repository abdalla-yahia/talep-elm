    ...
    <div className={`${style.userLink} z-50 flex justify-center items-center`}>
      {User?.name !== undefined && User?.name !== '' && User?.name !== null && User?.name !== 'AxiosError' ?
          ...
          <>
            <div className={`text-text_color flex justify-center items-center px-5 lg:flex-row flex-col`}>
              <Link href={role !== undefined ? `/${role}/dashboard/articles` : '/'} className="cursor-pointer flex flex-col justify-center items-center text-center">
                ...
                <p className="rounded-full hover:text-text_color text-text_color">{(User?.name)}</p>
              </Link>
              <button onClick={() => { LogoutHandeller(); setToggle(!toggle) }} className="hover:bg-text_color bg-second_background_color hover:text-second_background_color mx-2 p-2 rounded">
                خروج
              </button>
            </div>
          </>
        ...
        (<button onClick={() => setToggle(!toggle)} className="p-2 rounded ">
          <Link className="p-2 rounded hover:bg-second_background_color bg-background_color" href="/login"> تسجيل الدخول</Link>
        </button>)}
    </div>
  )
}

            ...
            <div className="card w-full relative flex justify-center items-start pt-3">
                ...
                <div className="flex md:flex-row lg:flex-row flex-col-reverse items-center gap-3 justify-between w-full pl-3">
                    ...
                    <div className="card-image">
                        <Image loading="lazy" width={150} height={150} src={Owner?.Owner?.image && `${Owner?.Owner?.image}` || img.male_admin} alt="profile" className="rounded-[20%]" />
                    </div>
                </div>
                <div className="card-foot text-sm bg-gray-500 w-full flex justify-center items-center">
                    <p className="card-text text-primary_color">تم الاضافة في {DateConvert(Owner?.Owner?.createdAt)} </p>
                </div>
                {/* Edit Personal Data */}
                {toggle && <div className="edit z-50 bg-green-200 absolute w-full top-0 left-0">
                    ...
                    <button onClick={() => UpdateOwnerHAndeller()} className="w-full p-2 bg-orange-500 text-text_color">حفظ البيانات</button>
                </div>}
                ...
                <icon.CiTrash title="حذف حسابك نهائياً" onClick={() => { DeleteOwnerHandeller() }} className=" absolute top-1 left-2 text-xl text-accent_color p-0 cursor-pointer" />
            </div>
        </div>
    )
}

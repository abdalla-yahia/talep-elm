...
export async function GET(request:NextRequest,{ params }: { params: Promise<{ id: string }> }):Promise<NextResponse>{
    try {
        ...
        const FindComments = await prisma.comments.findUnique({
...
export async function PUT(request:NextRequest,{ params }: { params: Promise<{ id: string }> }):Promise<NextResponse>{
    try {
        ...
        if(!FindComments){
            ...
            const UpdateComments = await prisma.comments.update({where:{id:parseInt(id)},data:{
                text:body.text,
            }})
            return NextResponse.json({message:'Success To Update Comments',UpdateComments,status:201},{status:201})
    } catch (error) {
        return NextResponse.json({message:'Something Wen Wrong',error,status:400},{status:400})
    }
}
 ...
 * @access Privite All Users
 * @path ~/api/v1/comments/:id
 * @returns Delete A Specific Comments Data
 */
...
export async function DELETE(request:NextRequest,{ params }: { params: Promise<{ id: string }> }):Promise<NextResponse>{
    try {
        ...
        const FindComments = await prisma.comments.findUnique({where:{id:parseInt(id)}})
        if(!FindComments){
            return NextResponse.json({message:'Comment Not Found'},{status:404})
            }
        if(UserFromToken.id !== FindComments?.userId && Role(UserFromToken.role) < 1){
            ...
            await prisma.comments.delete({where:{id:parseInt(id)}})

...
export default function ExamInfo() {
    const {DeleteExamResult}  =useAppSelector(state=>state.examResult) as unknown as {DeleteExamResult:{status:number}}
    const {AllUsers} = useAppSelector(state=>state.user) as unknown as {AllUsers:{status:number,User:User[]}}
    const {Exam} = useAppSelector(state=>state.exam) as unknown as {Exam:{status:number,title:string,ExamResult: ExamResultInterface[]}}
  ...
  return (
    ...
    <div className="w-full">
        <div className="flex flex-col justify-center items-center mb-4 w-full text-gray-700">
            <h2 className="text-2xl font-bold w-full justify-center my-2  items-center">معلومات الإختبار</h2>
            ...
            <div className="flex items-start justify-start w-full gap-2  my-1">
                ...
                <h3 className="ml-2 text-orange-700 ">{`( ${(( +Exam?.ExamResult?.length / +AllUsers?.User?.length ) * 100).toFixed(2)} % )  طالب / طالبة`} </h3>
            </div>
            <div className="flex items-start justify-start w-full gap-2  my-1">
                <h3> عدد من لم يمتحنوا الإختبار   : </h3>
                <h3 className="ml-2 text-yellow-700 ">{`( ${( +AllUsers?.User?.length - +Exam?.ExamResult?.length )} )  طالب / طالبة`} </h3>
            </div>
        </div>
    </div>
    <div className="table border-1 border-blue-400">
        <div className="table-header-group bg-blue-300 text-red-700 ">
            ...
            <span className="table-cell text-center border-1 py-1 border-blue-400">موعد اداء الإختبار</span>
            ...
            Exam?.ExamResult?.map((assRis, index) => (
                <div key={index} className={`${index % 2 === 0 ? 'bg-yellow-50 text-gray-700' :'bg-green-100 text-red-800'} table-row-group border-1 border-blue-600`}>
                    ...
                    <span className="table-cell text-center border-1 border-blue-300">
                        <icon.CiTrash  onClick={()=>DeleteScoreUserHandeller(assRis?.id as unknown as string)} title="حذف درجة الطالب" className="hover:text-red-600 hover:text-xl transition-all p-0 bg-transparent border-none w-full cursor-pointer"/>
                    </span>
                    </div>
            ))
        }

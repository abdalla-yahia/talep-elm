  ...
  return (
    ...
    <div className="card w-full relative flex justify-center items-start pt-3">
        ...
        <div className="card-foot text-sm bg-gray-500 w-full flex justify-center items-center">
            <p className="card-text text-slate-100">تم الاضافة في {DateConvert(Teacher?.Teacher?.createdAt)} </p>
        </div>
    {/* Edit Personal Data */}
       {toggle && <div className="edit z-40 bg-green-200 absolute w-full top-0 left-0">
        ...
        <button onClick={()=>UpdateTeacherHAndeller()} className="w-full p-2 bg-orange-500 text-white">حفظ البيانات</button>
        </div>}
    {/* Button On Click It Show Card Edit Or Delete Teacher Account*/}
        ...
        <icon.CiTrash title="حذف حسابك نهائياً" onClick={()=>{DeleteTeacherHandeller()}} className=" absolute top-1 left-2 text-xl text-red-500 p-0 cursor-pointer" />
        </div>
    </div>
  )
}

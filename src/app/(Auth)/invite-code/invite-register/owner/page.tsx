...
import { createOwner } from "@/lib/Actions/OwnersActions";
import { Gender } from "@prisma/client";
import { permation } from "@/Interfaces/InterFaces";
// import { permation } from "@/Interfaces/InterFaces";

export default function OwnerRegister({permation}:{permation:permation|null}) {
  const {CreateOwner} = useAppSelector(state => state.owner) as unknown as {CreateOwner:{status:number}}
...
useEffect(()=>{
if(CreateOwner?.status){
  if(CreateOwner?.status === 201){
    ...
    setAge('')
    setEducation('')
    redirect('/owner/dashboard/articles')
  }else if(CreateOwner?.status === 400){
  ...
  return (
    <>
    <FullTitle F_Title={'إنشاء مالك جديد'}/>
    <div className="container w-full md:w-2/3 lg:w-1/2 flex-col justify-center items-center mt-2">
      <div className="flex justify-between items-center relative gap-2 w-full ">
        ...
        <h2 className="hidden md:block lg:block ">اسم المالك</h2>
        <input value={name} onChange={(e)=>setUsername(e?.target?.value)} type="text" name="" id="" className="rounded px-2 my-1 caret-red-700 w-full  md:w-2/3 lg:w-2/3  text-gray-700"   placeholder="ادخل اسم المالك" />
      </div>
      <div className="flex justify-between items-center relative gap-2 w-full ">
        <span className="text-red-600 absolute left-1 top-0">*</span>
        <h2 className="hidden md:block lg:block ">جنس المالك</h2>
        <select value={gender} onChange={(e)=>setGender(e?.target?.value)}  name="" id="" className="rounded px-2 my-1 caret-red-700 w-full  md:w-2/3 lg:w-2/3  text-gray-700"    >
          <option selected disabled value=""> اختر جنس المالك</option>
      ...
      <div className="flex justify-between items-center relative gap-2 w-full ">
        <span className="text-red-600 absolute left-1 top-0">*</span>
        <h2 className="hidden md:block lg:block ">ايميل المالك</h2>
        <input value={email} onChange={(e)=>setEmail(e?.target?.value)} type="email" name="" id="" className="rounded px-2 my-1 caret-red-700 w-full  md:w-2/3 lg:w-2/3  text-gray-700"   placeholder="ادخل ايميل المالك" />
      </div>
      <div className="flex justify-between items-center relative gap-2 w-full ">
        <span className="text-red-600 absolute left-1 top-0">*</span>
        <h2 className="hidden md:block lg:block ">رقم هاتف المالك</h2>
        <input value={phone} onChange={(e)=>setPhone(e?.target?.value)} type='tel' dir="rtl" name="" id="" className="rounded px-2 my-1 caret-red-700 w-full  md:w-2/3 lg:w-2/3  text-gray-700"   placeholder="ادخل رقم هاتف المالك" />
      </div>
      <div className="flex justify-between items-center relative gap-2 w-full ">
        <span className="text-red-600 absolute left-1 top-0">*</span>
    ...
    <button onClick={()=>handleRegister()} className="w-full cursor-pointer p-2 bg-orange-600 hover:bg-orange-800 rounded">إنشاء حساب مالك جديد</button>
    </div>

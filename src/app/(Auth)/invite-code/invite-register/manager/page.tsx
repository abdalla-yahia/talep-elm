...
import { createManager } from "@/lib/Actions/ManagersActions";
import { fetchAllOwners } from "@/lib/Actions/OwnersActions";
import { Gender, Owner } from "@prisma/client";
import { permation } from "@/Interfaces/InterFaces";


export default function ManagerRegister({permation}:{permation:permation|null}) {
  const {AllOwners}  = useAppSelector(state=>state.owner) as unknown as  {AllOwners:{Owners:Owner[]}}
  const {CreateManager} = useAppSelector(state=>state.manager) as unknown as {CreateManager:{status:number}}
  ...
  useEffect(()=>{
    if(!permation){
      redirect('/')
    }else if(permation?.role !== 'MANAGER'){
      redirect('/')
    }
  },[permation])
...
const handleRegister = () => {
  if(name !== '' && gender !== ''  && email !== '' && phone !== '' && password !== ''&& ownerId !== ''){
...
if(CreateManager?.status){
  if(CreateManager?.status === 201){
    ...
    setAge('')
    setEducation('')
    redirect('/managers/dashboard/articles')
  }else if(CreateManager?.status === 400){
  ...
  return (
    <>
    <FullTitle F_Title={'إنشاء مدير جديد'}/>
    <div className="container w-full md:w-2/3 lg:w-1/2 flex-col justify-center items-center mt-2">
      <div className="flex justify-between items-center relative gap-2 w-full ">
        ...
        <h2 className="hidden md:block lg:block ">اسم المدير</h2>
        <input value={name} onChange={(e)=>setUsername(e?.target?.value)} type="text" name="" id="" className="rounded px-2 my-1 caret-red-700 w-full  md:w-2/3 lg:w-2/3  text-gray-700"   placeholder="ادخل اسم المدير" />
      </div>
      <div className="flex justify-between items-center relative gap-2 w-full ">
        <span className="text-red-600 absolute left-1 top-0">*</span>
        <h2 className="hidden md:block lg:block ">جنس المدير</h2>
        <select value={gender} onChange={(e)=>setGender(e?.target?.value)}  name="" id="" className="rounded px-2 my-1 caret-red-700 w-full  md:w-2/3 lg:w-2/3  text-gray-700"    >
          <option selected disabled value=""> اختر جنس المدير</option>
      ...
      <div className="flex justify-between items-center relative gap-2 w-full ">
        <span className="text-red-600 absolute left-1 top-0">*</span>
        <h2 className="hidden md:block lg:block ">المالك</h2>
        <select value={ownerId} disabled={gender === '' ? true : false} onChange={(e)=>setOwnerId(e?.target?.value)}  name="" id="" className="rounded px-2 my-1 caret-red-700 w-full  md:w-2/3 lg:w-2/3  text-gray-700"    >
          <option selected disabled value=""> اختر  المالك</option>
            {
              AllOwners?.Owners?.length > 0 && AllOwners?.Owners?.filter(el=>el.gender == gender)?.map((item, index) => {
                return (
      ...
      <div className="flex justify-between items-center relative gap-2 w-full ">
        <span className="text-red-600 absolute left-1 top-0">*</span>
        <h2 className="hidden md:block lg:block ">ايميل المدير</h2>
        <input value={email} onChange={(e)=>setEmail(e?.target?.value)} type="email" name="" id="" className="rounded px-2 my-1 caret-red-700 w-full  md:w-2/3 lg:w-2/3  text-gray-700"   placeholder="ادخل ايميل المدير" />
      </div>
      <div className="flex justify-between items-center relative gap-2 w-full ">
        <span className="text-red-600 absolute left-1 top-0">*</span>
        <h2 className="hidden md:block lg:block ">رقم هاتف المدير</h2>
        <input value={phone} onChange={(e)=>setPhone(e?.target?.value)} type='tel' dir="rtl" name="" id="" className="rounded px-2 my-1 caret-red-700 w-full  md:w-2/3 lg:w-2/3  text-gray-700"   placeholder="ادخل رقم هاتف المدير" />
      </div>
      <div className="flex justify-between items-center relative gap-2 w-full ">
        <span className="text-red-600 absolute left-1 top-0">*</span>
    ...
    <button onClick={()=>handleRegister()} className="w-full cursor-pointer p-2 bg-orange-600 hover:bg-orange-800 rounded">إنشاء حساب مدير  جديد</button>
    </div>

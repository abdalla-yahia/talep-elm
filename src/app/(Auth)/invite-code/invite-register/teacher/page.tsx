...
import { createTeacher } from "@/lib/Actions/TeachersActions";
import { fetchSubjects } from "@/lib/Actions/SubjectsActions";
import { AdminTeacher, Groups, Subjects } from "@prisma/client";
import { permation } from "@/Interfaces/InterFaces";


export default function TeacherRegister({permation}:{permation:permation|null}) {
  ...
  const {CreateTeacher} = useAppSelector(state=>state.teacher) as unknown as {CreateTeacher:{status:number}}
  ...
  useEffect(()=>{
    ...
    else if(permation?.role !== 'TEACHER'){
      redirect('/')
    }
  },[permation])
...
if(CreateTeacher?.status){
  if(CreateTeacher?.status === 201){
    ...
    setAge('')
    setEducation('')
    redirect('/teachers/dashboard/articles')
  }else if(CreateTeacher?.status === 400){
  ...
  return (
    <>
    <FullTitle F_Title={'إنشاء مدرس جديد'}/>
    <div className="container w-full md:w-2/3 lg:w-1/2 flex-col justify-center items-center mt-2">
      <div className="flex justify-between items-center relative gap-2 w-full ">
        ...
        <h2 className="hidden md:block lg:block ">اسم المدرس</h2>
        <input value={name} onChange={(e)=>setUsername(e?.target?.value)} type="text" name="" id="" className="rounded px-2 my-1 caret-red-700 w-full  md:w-2/3 lg:w-2/3  text-gray-700"   placeholder="ادخل اسم المدرس" />
      </div>
      <div className="flex justify-between items-center relative gap-2 w-full ">
        <span className="text-red-600 absolute left-1 top-0">*</span>
        <h2 className="hidden md:block lg:block ">جنس المدرس</h2>
        <select value={gender} onChange={(e)=>setGender(e?.target?.value)}  name="" id="" className="rounded px-2 my-1 caret-red-700 w-full  md:w-2/3 lg:w-2/3  text-gray-700"    >
          <option selected disabled value=""> اختر جنس المدرس</option>
      ...
      <div className="flex justify-between items-center relative gap-2 w-full ">
        <span className="text-red-600 absolute left-1 top-0">*</span>
        <h2 className="hidden md:block lg:block ">المادة</h2>
        <select value={SubjectId} disabled={gender === '' ? true : false} onChange={(e)=>setSubjectId(e?.target?.value)}  name="" id="" className="rounded px-2 my-1 caret-red-700 w-full  md:w-2/3 lg:w-2/3  text-gray-700"    >
          <option selected disabled value=""> اختر المادة</option>
            {
              AllSubjects?.Subjects?.length > 0 && AllSubjects?.Subjects?.map((item, index) => {
                return (
      ...
      <div className="flex justify-between items-center relative gap-2 w-full ">
        <span className="text-red-600 absolute left-1 top-0">*</span>
        <h2 className="hidden md:block lg:block ">ايميل المدرس</h2>
        <input value={email} onChange={(e)=>setEmail(e?.target?.value)} type="email" name="" id="" className="rounded px-2 my-1 caret-red-700 w-full  md:w-2/3 lg:w-2/3  text-gray-700"   placeholder="ادخل ايميل المدرس" />
      </div>
      <div className="flex justify-between items-center relative gap-2 w-full ">
        <span className="text-red-600 absolute left-1 top-0">*</span>
        <h2 className="hidden md:block lg:block ">رقم هاتف المدرس</h2>
        <input value={phone} onChange={(e)=>setPhone(e?.target?.value)} type='tel' dir="rtl" name="" id="" className="rounded px-2 my-1 caret-red-700 w-full  md:w-2/3 lg:w-2/3  text-gray-700"   placeholder="ادخل رقم هاتف المدرس" />
      </div>
      <div className="flex justify-between items-center relative gap-2 w-full ">
        <span className="text-red-600 absolute left-1 top-0">*</span>
    ...
    <button onClick={()=>handleRegister()} className="w-full cursor-pointer p-2 bg-orange-600 hover:bg-orange-800 rounded">إنشاء حساب مدرس جديد</button>
    </div>
